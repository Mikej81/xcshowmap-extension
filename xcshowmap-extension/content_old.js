...
chrome.runtime.onMessage.addListener((message) => {
    ...
    if (message.action === "csrfTokenCaptured") {
        ...
        console.log(`🔑 [CONTENT] ${tokenType} CSRF Token captured at ${new Date().toISOString()}! Ready for manual refresh.`);
        // Removed automatic fetch to prevent excessive API calls
    }
...
// Initial attempt to fetch data with improved timing - DISABLED to prevent automatic calls
// Only fetch data when explicitly requested by user action
function attemptInitialFetch() {
    console.log(`🚀 [CONTENT] Initial fetch disabled to prevent excessive API calls`);
    ...
    // Do not automatically fetch - wait for user action
    console.log(`⏸️ [CONTENT] Waiting for user action to fetch data (refresh button)`);
}

// Disabled automatic fetching to prevent 429 errors
console.log(`⏸️ [CONTENT] Automatic fetching disabled - use refresh button to load data`);
...
// Navigation change detection DISABLED to prevent excessive API calls
// Users should manually refresh data when needed
let lastUrl = window.location.href;
// Removed automatic polling to prevent 429 errors
